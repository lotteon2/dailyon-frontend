...
const fetchDefaultData = async () => {
  const commentPageResponse = await getComments(
    postId.value,
    0,
    requestSize.value,
    requestSort.value
...
const onOpenReplyCommentInput = async (commentId: number) => {
  isOpenReplyCommentInput.value.has(commentId)
    ? isOpenReplyCommentInput.value.delete(commentId)
    : isOpenReplyCommentInput.value.add(commentId)
}
...
const onSubmitReplyComment = debounce(async (commentId: number) => {
  if (!isCommentRegistered.value) {
    if (memberId === null) {
      ...
      createReplyCommentRequest.value.description.length < 5 ||
      createReplyCommentRequest.value.description.length > 140
    ) {
