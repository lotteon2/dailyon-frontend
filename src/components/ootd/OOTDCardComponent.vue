    ...
    <div class='ootd-header-container'>
      ...
      <div class='ootd-header-bar-wrapper'>
        <div class='ootd-header-sort-wrapper'>
          <div v-for='sortOption in sortOptions' :key='sortOption.value' class='ootd-sort-wrapper'>
            <div class='ootd-sort-text' @click='onChangeSort(sortOption.value)'
                 :class="{ 'selected': requestSort === sortOption.value }">
              {{ sortOption.label }}
            </div>
          </div>
        </div>
          ...
          <div class='ootd-post-card-like-wrapper' @click='likeButtonClickListener(post.id, post.isLike)'>
            <svg class='ootd-post-card-like' xmlns='http://www.w3.org/2000/svg' width='44' height='42'
              ...
              <path class='ootd-post-card-like-icon'
                    :class="{ 'selected': post.isLike === undefined ? false : post.isLike }"
