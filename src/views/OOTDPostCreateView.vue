  ...
  if (hashTagNames.length < 3) {
    if (hashTagInput.value.length < 1 || hashTagInput.value.length > 10) {
      ...
      if (hashTagNames.indexOf(hashTagInput.value) !== -1) {
        await warningModal('알림', '이미 추가된 해시태그 입니다.')
      } else {
    ...
    await warningModal('알림', '해시태그는 최대 3개까지 입력 가능합니다.')
  }
}

const removeHashTag = async () => {
  if (postCreateRequest.value.hashTagNames && postCreateRequest.value.hashTagNames.length !== 0) {
...
const onPostImgClick = async (event: any) => {
  if (temporaryTagProducts.value.length >= 5) {
    await warningModal('알림', '상품 태그는 최대 5개까지 태그할 수 있습니다.')
  } else {
...
const temporaryTagProducts = ref<Array<TemporaryCreateTagProduct>>(
  new Array<TemporaryCreateTagProduct>()
)
const onSelectBtnClick = async (
  ...
  if (temporaryTagProducts.value.length >= 5) {
    ...
    const temporaryTagProductIndex = temporaryTagProducts.value.findIndex(
      (temporaryTagProduct) => temporaryTagProduct.id === productId
    )
...
const onSubmit = async () => {
  ...
  if (isConfirmed) {
    ...
    if (hashTagNames.length < 1 || hashTagNames.length > 3) {
      await warningModal('알림', '해시태그는 최소 1개 최대 3개까지 입력 가능합니다.')
      return
    }
    ...
    if (postImgName === '' || postThumbnailImgName === '' || inputPostImgFile.value === undefined) {
      await warningModal('알림', '게시글 이미지를 등록해주세요.')
      return
    }
  ...
  <div class='frame'>
        ...
        <div class='div-3'>
          <div class='div-4'>
            ...
            <input
              ...
              type='text'
              maxlength='3'
              v-model='postCreateRequest.stature'
              placeholder='키 입력(선택사항)'
            />
          ...
          <div class='div-5'>
            ...
            <input
              ...
              type='text'
              maxlength='3'
              v-model='postCreateRequest.weight'
              placeholder='몸무게 입력(선택사항)'
            />
            ...
            <div class='div-7'>
              <div v-for='temporaryTagProduct in temporaryTagProducts' class='div-wrapper'>
                <div class='text-wrapper-5'>
                  <span>{{ temporaryTagProduct.name }}</span
                  >-<span>{{ temporaryTagProduct.sizeName }}</span>
                </div>
              </div>
            </div>
          </div>
          <div class='div-8'>
            ...
            <div class='div-9'>
              ...
              <div class='div-11'>
                ...
                <div
                  v-if='postCreateRequest.hashTagNames.length < 3'
                  class='div-wrapper-4 hashtag-add-btn'
                >
                  <div class='text-wrapper-7' @click='addHashTag'>추가</div>
                </div>
                <div
                  v-if='postCreateRequest.hashTagNames.length !== 0'
                  class='div-wrapper-4 hashtag-remove-btn'
                >
    ...
    <div class='div-12'>
      ...
      <div v-if="postCreateRequest.postImgName !== ''" class='post-img-wrapper'>
        ...
        <div
          v-for='(temporaryTagProduct, index) in temporaryTagProducts'
          class='product-detail-tag-wrapper'
          :key='temporaryTagProduct.id'
          :style='{
            left: `${temporaryTagProduct.leftGapPercent}%`,
            top: `${temporaryTagProduct.topGapPercent}%`
          }'
        >
          <svg
          ...
          <div class='product-detail-tag-dropdown-wrapper' :style='{ zIndex: 5 - index }'>
            <svg
              class='product-detail-tag-dropdown-pointer'
              xmlns='http://www.w3.org/2000/svg'
              width='33'
              height='36'
              viewBox='0 0 33 36'
            ...
            <div class='product-detail-tag-dropdown-box'>
              <img
                class='product-image'
                :src='`${VITE_STATIC_IMG_URL}${temporaryTagProduct.imgUrl}`'
              />
              ...
              <div class='left-wrapper'>
                <div class='div-wrapper-4 hashtag-remove-btn'>
                  <div
                    ...
                    @click='removeTagProduct(temporaryTagProduct.id, temporaryTagProduct.sizeName)'
                  >
                    삭제
                  </div>
                </div>
    ...
    <div class='button-container'>
      <div class='cancel-button'>
        <span class='cancel-button-text' @click='onCancel'> 작성 취소 </span>
      </div>
      <div class='submit-button' @click='onSubmit'>
        <span class='submit-button-text'> 작성 완료 </span>
      </div>
    </div>
  </div>
</template>
