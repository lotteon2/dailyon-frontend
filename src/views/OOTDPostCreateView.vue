...
import { ProductSearchResponse } from '@/apis/ootd/PostDto'
...
const postCreateRequest = ref<PostCreateRequest>({
  title: String(),
  description: String(),
  stature: String(),
  weight: String(),
  ...
  postImgName: String(),
  postImageProductDetails: Array<PostImageProductDetailCreateRequest>()
})
...
const handlePostImgChange = async (inputFile: File) => {
...
const readFile = async (file: File) => {
  ...
  reader.onload = (e) => {
    inputPostImg.value = e.target.result
  }
...
const removeHashTag = async () => {
  if(postCreateRequest.value.hashTagNames.length !== 0) {
...
const currentLeftGapPercent = ref<number>()
const currentTopGapPercent = ref<number>()
const onPostImgClick = async (event) => {
      ...
      <div class='div-2-text-wrapper'>
        ...
        <div class='div-3'>
          <div class='div-4'>
            ...
            <div v-if='!validationMessage.stature.number.isValid' class='validation-wrapper'>{{ validationMessage.stature.number.message }}</div>
          ...
          <div class='div-5'>
            ...
            <div v-if='!validationMessage.weight.number.isValid' class='validation-wrapper'>{{ validationMessage.weight.number.message }}</div>
          </div>
          <div class='div-6'>
            <div class='text-wrapper-4'>상품정보</div>
            <div class='div-7'>
              <div v-for='temporaryTagProduct in temporaryTagProducts' class='div-wrapper'>
                <div class='text-wrapper-5'><span>{{ temporaryTagProduct.name }}</span>-<span>{{ temporaryTagProduct.sizeName }}</span></div>
              </div>
            </div>
          </div>
          <div class='div-8'>
    ...
    <div class='div-12'>
      ...
      <div v-if='postCreateRequest.postImgName !== ""' class='post-img-wrapper' >
        ...
        <div v-for='temporaryTagProduct in temporaryTagProducts'
             class='product-detail-tag-wrapper' :style="{ left: `${temporaryTagProduct.leftGapPercent}%`, top: `${temporaryTagProduct.topGapPercent}%` }">
              ...
              <div class='left-wrapper'>
                <div class='div-wrapper-4 hashtag-remove-btn'>
                  <div class='text-wrapper-7'
                       @click='removeTagProduct(temporaryTagProduct.id, temporaryTagProduct.sizeName)'>삭제</div>
                </div>
